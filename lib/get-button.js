...
module.exports = async(fileKey, URLformat, page, { color, effect }, headers) => {
	const result = {
    sets: {
      common: {
        fontFamily: {
      ...
      if (propName === 'size') {
        ...
        if (variantContentText) {
          ...
          if (textEl) for (let i = 0; i < styles.length; i++) {
            ...
            try {
              const textStyleNode = await getNodeId(textEl.styles.text, fileKey, URLformat, headers);
              const styleName = naming(textStyleNode.document.name, true).split('/')[0];
              value = `{text.${styleName}.${el}.value}`
            } catch (error) {
