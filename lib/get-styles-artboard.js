 ...
 * @returns {Object}
 */
module.exports = async ({ fileKey, config, params }) => {
	const typeIsFiles = type === 'files';
	const isJoinPlatformMode = params.platformMode === 'join';
	...
	['text', 'button', 'grid'].forEach(el => {
		...
		if (Object.entries(elSets).length) {
			...
			isJoinPlatformMode && (data[el].sets = sets.subtractSets(elSets, config.platforms));
		}
	});
