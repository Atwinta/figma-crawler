...
module.exports = async ({ fileKey, URLformat, config, params, headers }) => {
	...
	buttonPage && (data.button = await getButton(fileKey, URLformat, buttonPage, data, headers));

	params.mode === 'join' && ['text', 'button', 'grid'].forEach(el => {
		const elSets = data[el].sets || {};
		Object.entries(elSets) && (data[el].sets = sets.subtractSets(elSets, config.platforms));
	});
