...
// const getGrids = require('./get-grids');
 ...
 * @returns
 */
module.exports = async function(key, URLformat, { colorFormat }, headers) {
	...
	['text', 'button'].forEach(el => {
		const elSets = data[el].sets || {};

		if (Object.entries(elSets)) {
			/**
			 ...
			 */
			const subsctedSets = sets.subtractSets(elSets);
			for (const key in subsctedSets) {
				data[el].sets[key] && (data[el].sets[key] = subsctedSets[key]);
			}
		}
	});
